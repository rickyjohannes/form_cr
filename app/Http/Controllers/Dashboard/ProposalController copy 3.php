...
class ProposalController extends Controller
{
    ...
    public function index_cr() 
    {
        ...
        // Mengambil proposal yang memiliki status_barang 'Change Request'
        $proposals = Proposal::where('status_barang', 'Change Request')->get();
    
        // Cek role dan jika ada data yang cocok
        if ($role == 'user' && $proposals->isNotEmpty()) {
            // Jika role adalah 'user' dan ada proposal dengan status 'Change Request'
            return $this->user();
        } elseif ($role == 'it' && $proposals->isNotEmpty()) {
            // Jika role adalah 'it'
            return $this->it();
        } else {
    ...
    private function user()
    {
        ...
        // Mengambil proposal berdasarkan user_id dan status_barang 'Change Request'
        $proposals = Proposal::where('user_id', $id)
                             ->where('status_barang', 'Change Request') // Menambahkan kondisi untuk status 'Change Request'
                             ->get();
        ...
        if (request()->routeIs('proposal.*')) {
            ...
            $view = 'dashboard.proposal.user';  // Default view jika tidak ada yang cocok
        }
    ...
    private function it()
    {
        ...
        if (request()->routeIs('proposal.*')) {
            ...
            $view = 'dashboard.proposal.it';  // Default view jika tidak ada yang cocok
        }
